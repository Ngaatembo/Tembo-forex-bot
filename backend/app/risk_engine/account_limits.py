...
def account_data_valid(account: AccountState) -> tuple[bool, str]:
    ...
    if not isinstance(account.open_positions_count, int) or isinstance(account.open_positions_count, bool) or account.open_positions_count < 0:
        return False, "Open position count is missing, non-integer, or negative."
    if not isfinite(account.total_open_risk_pct) or not (0 <= account.total_open_risk_pct <= 1):
        return False, "Total open risk state is non-finite or outside the valid 0–100% range."
    if not isfinite(account.daily_realized_pnl) or not isfinite(account.daily_unrealized_pnl):
        return False, "Daily realized/unrealized P&L is non-finite."
    if not isinstance(account.kill_switch_active, bool):
        return False, "Kill switch state is invalid; refusing to assume it is safe."
...
def check_per_trade_risk(risk_pct: float, limits: RiskLimitsConfig) -> tuple[bool, str]:
    if not isfinite(risk_pct) or not (0 <= risk_pct <= 1):
        return False, "Proposed risk percentage is invalid or non-finite."
...
def check_total_open_risk(account: AccountState, new_trade_risk_pct: float, limits: RiskLimitsConfig) -> tuple[bool, str]:
    if not isfinite(account.total_open_risk_pct) or not (0 <= account.total_open_risk_pct <= 1):
        return False, "Existing total open risk state is invalid or non-finite."
    if not isfinite(new_trade_risk_pct) or not (0 <= new_trade_risk_pct <= 1):
        return False, "New trade risk percentage is invalid or non-finite."
...
def check_daily_loss(account: AccountState, limits: RiskLimitsConfig) -> tuple[bool, str]:
    if not isfinite(account.daily_realized_pnl) or not isfinite(account.daily_unrealized_pnl):
        return False, "Daily P&L is invalid or non-finite."
    ...
    if not isfinite(loss_pct):
        return False, "Daily loss percentage is non-finite."
...
def check_drawdown(account: AccountState, limits: RiskLimitsConfig) -> tuple[bool, str]:
    if not isfinite(account.equity) or not isfinite(account.peak_equity) or account.peak_equity <= 0:
        return False, "Drawdown inputs are invalid or non-finite."
    drawdown_pct = (account.peak_equity - account.equity) / account.peak_equity
    if not isfinite(drawdown_pct) or drawdown_pct < 0:
        return False, "Drawdown calculation is invalid or non-finite."
...
def check_position_limit(account: AccountState, limits: RiskLimitsConfig) -> tuple[bool, str]:
    if not isinstance(account.open_positions_count, int) or isinstance(account.open_positions_count, bool) or account.open_positions_count < 0:
        return False, "Open position count is invalid."
...
def check_exposure(position_size: float, entry_price: float, account: AccountState, limits: RiskLimitsConfig) -> tuple[bool, str]:
    if not isfinite(position_size) or position_size <= 0 or not isfinite(entry_price) or entry_price <= 0:
        return False, "Position size or entry price is invalid or non-finite."
    if not isfinite(account.equity) or account.equity <= 0:
        return False, "Account equity is invalid or non-finite."
    notional = position_size * entry_price
    exposure_pct = notional / account.equity
    if not isfinite(exposure_pct) or exposure_pct < 0:
        return False, "Exposure calculation is invalid or non-finite."
